...
const bundledPackageRoot = path.resolve(scriptDirectory, "..", "..", "..");
...
if (!options.source) {
  options.source = bundledPackageRoot;
  if (!fs.existsSync(path.join(options.source, ".codex-plugin", "plugin.json"))) {
    options.source = process.env.PLUGIN_ROOT || process.env.CLAUDE_PLUGIN_ROOT || "";
  }
}
if (!options.source) {
  throw new Error("source is required: pass --source or invoke the skill from a complete Git Workflow package");
}
