...
python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); assert d["version"] == "2.6.0"; assert len(d["managed_files"]) == 8' "$EMPTY_TARGET/.git-workflow/version.json" || fail "invalid version ledger"
...
[ "$(find "$CLAUDE_TARGET/.claude/skills" -name SKILL.md | wc -l | tr -d ' ')" = "21" ] || fail "Claude install did not create 21 skills"
